...
-- Rode no Supabase SQL Editor antes de subir/testar a v34.
...
insert into public.establishments (id, profile_id, trade_name, legal_name, cnpj, document_number, phone, whatsapp, city, state, status, base_address, distance_radius_km, address_status, city_id, created_at)
values (
  ...
  'Centro, Taquaritinga/SP', 3, 'aprovado', '10000000-0000-4000-8000-000000000001', now()
)
on conflict (id) do update set
...
-- v34: administração de admins e reset de senha sem expor senha antiga.
...
begin
  if length(coalesce(p_password, '')) < 4 or length(coalesce(p_password, '')) > 6 then
    raise exception 'Senha deve ter de 4 a 6 números';
  end if;
  ...
  insert into public.app_users (city_id, role, username, email, display_name, password_hash, entity_id, is_active, must_change_password)
  values (v_city_id, v_role, lower(trim(p_username)), nullif(p_email, ''), p_display_name, public.chm_password_hash(p_username, p_password), null, true, false)
  on conflict (city_id, username) do update set
    role = excluded.role,
    email = excluded.email,
    display_name = excluded.display_name,
...
grant execute on function public.chm_create_admin_user(text, text, text, text, text, text) to anon, authenticated;
...
declare
  v_username text;
begin
  if length(coalesce(p_new_password, '')) < 4 or length(coalesce(p_new_password, '')) > 6 then
    raise exception 'Senha deve ter de 4 a 6 números';
  end if;
  ...
  update public.app_users
     set password_hash = public.chm_password_hash(v_username, p_new_password), must_change_password = false
   where id = p_user_id;
  return true;
end;
$$;
...
declare
  v_user_id uuid;
begin
  if p_role not in ('estabelecimento','motoboy') then
    raise exception 'Perfil inválido';
  end if;
  select id into v_user_id
    from public.app_users
   where role = p_role and entity_id = p_entity_id
   limit 1;
...
-- Garante que os usuários de teste estejam com telefone para login por WhatsApp/celular.
update public.app_users set phone = '17999990001' where username = 'speed_teste';
update public.app_users set phone = '17999990002' where username = 'motoca_teste';

-- ==========================================================
-- Chegou Motoca v35
-- Correções: login de estabelecimento, superadmin/admin local,
-- WhatsApp do admin, senha forte para admin e logout/segurança.
-- Pode rodar após v34; é reexecutável.
-- ==========================================================
...
-- reforça vínculo do usuário de teste do estabelecimento
update public.establishments
   set profile_id = '30000000-0000-4000-8000-000000000002',
       trade_name = 'Speed Teste Taquaritinga',
       legal_name = 'Speed Teste Taquaritinga',
       cnpj = '00000000000191',
       ...
       base_address = coalesce(base_address, 'Centro, Taquaritinga/SP'),
       address_status = 'aprovado'
 where id = '40000000-0000-4000-8000-000000000001';

update public.app_users
   set role = 'estabelecimento',
       city_id = '10000000-0000-4000-8000-000000000001',
       username = 'speed_teste',
       email = 'speed_teste@chegoumotoca.local',
       phone = '17999990001',
       document_number = '00000000000191',
       display_name = 'Speed Teste Taquaritinga',
       password_hash = public.chm_password_hash('speed_teste', '123456'),
       entity_id = '40000000-0000-4000-8000-000000000001',
       is_active = true,
       must_change_password = false
 where lower(username) = 'speed_teste';

update public.app_users
   set role = 'motoboy',
       city_id = '10000000-0000-4000-8000-000000000001',
       username = 'motoca_teste',
       email = 'motoca_teste@chegoumotoca.local',
       phone = '17999990002',
       document_number = '12345678909',
       display_name = 'Motoca Teste',
       password_hash = public.chm_password_hash('motoca_teste', '123456'),
       entity_id = '50000000-0000-4000-8000-000000000001',
       is_active = true,
       must_change_password = false
 where lower(username) = 'motoca_teste';

-- Login escolhe o usuário mais completo se houver duplicidade antiga.
create or replace function public.chm_login(p_identifier text, p_password text, p_city_slug text default 'taquaritinga-sp')
returns jsonb
language plpgsql
security definer
...
begin
  if length(coalesce(p_password, '')) < 8 then
    raise exception 'Senha administrativa deve ter pelo menos 8 caracteres';
  end if;
  ...
  select id into v_user_id
    from public.app_users
   where city_id = v_city_id and lower(username) = lower(trim(p_username))
   limit 1;
  ...
  if v_user_id is null then
    ...
    update public.app_users
       set role = v_role,
           ...
           password_hash = public.chm_password_hash(p_username, p_password),
           is_active = true,
           must_change_password = false
     where id = v_user_id;
  end if;
...
begin
  ...
  if v_actor.id is null or v_actor.role <> 'superadmin' then
    return false;
  end if;
...
create or replace function public.chm_reset_admin_password_secure(
...
as $$
begin
  if not public.chm_actor_is_superadmin(p_actor_user_id, p_actor_password) then
    raise exception 'Confirmação de superadmin inválida';
  end if;
  if not exists (select 1 from public.app_users where id = p_target_user_id and role in ('admin','superadmin')) then
    raise exception 'Administrador não encontrado';
  end if;
  perform public.chm_reset_app_user_password(p_target_user_id, p_new_password);
  insert into public.admin_action_logs(actor_user_id, target_user_id, action, details)
  values (p_actor_user_id, p_target_user_id, 'reset_admin_password', 'Senha administrativa redefinida pelo superadmin.');
  return true;
end;
$$;
...
create or replace function public.chm_set_admin_active_secure(
  p_actor_user_id uuid,
  p_actor_password text,
  p_target_user_id uuid,
  p_is_active boolean
...
begin
  ...
  update public.app_users
   ...
   where id = p_target_user_id and role in ('admin','superadmin');
  insert into public.admin_action_logs(actor_user_id, target_user_id, action, details)
  values (p_actor_user_id, p_target_user_id, case when p_is_active then 'activate_admin' else 'pause_admin' end, 'Status administrativo alterado.');
  return true;
end;
$$;
...
create or replace function public.chm_remove_admin_access_secure(
  p_actor_user_id uuid,
  p_actor_password text,
  p_target_user_id uuid
)
...
begin
  ...
  insert into public.admin_action_logs(actor_user_id, target_user_id, action, details)
  values (p_actor_user_id, p_target_user_id, 'remove_admin_access', 'Acesso de admin local removido/arquivado.');
  return true;
end;
$$;
...
-- v35 RLS/policies para novas estruturas
alter table public.admin_action_logs enable row level security;
drop policy if exists "admin_action_logs_mvp_all" on public.admin_action_logs;
create policy "admin_action_logs_mvp_all" on public.admin_action_logs for all to anon, authenticated using (true) with check (true);
